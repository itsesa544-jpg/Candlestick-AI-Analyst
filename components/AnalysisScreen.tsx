...
const getPredictionColorClasses = (prediction: 'Up' | 'Down' | 'Sideways') => {
    switch (prediction) {
      case 'Up': return 'text-green-700 border-green-500/50 bg-green-500/10';
      case 'Down': return 'text-red-700 border-red-500/50 bg-red-500/10';
      case 'Sideways': return 'text-yellow-700 border-yellow-500/50 bg-yellow-500/10';
      default: return 'text-gray-600 border-gray-500/50 bg-gray-500/10';
    }
}

export const AnalysisScreen: React.FC<AnalysisScreenProps> = ({ imageUrl, result, isLoading, error, onReset, platform, timeframe }) => {
  return (
    <div className="flex flex-col md:flex-row h-screen overflow-hidden bg-gray-100 text-gray-800">
      {/* Image Panel */}
      <div className="w-full md:w-1/2 h-1/3 md:h-full relative bg-gray-800">
        <img src={imageUrl} alt="Financial chart" className="w-full h-full object-contain" />
        <button
          onClick={onReset}
          className="absolute top-4 left-4 p-2 bg-white bg-opacity-50 rounded-full text-gray-700 hover:bg-gray-200 transition shadow-md"
          aria-label="Go back"
        >
          ...
          <div className="space-y-6">
            <div className={`p-4 rounded-lg border ${getPredictionColorClasses(result.prediction)}`}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-500 uppercase tracking-wide">Prediction</h3>
                  <p className="text-4xl font-bold">{result.prediction}</p>
                </div>
                <div className="ml-auto text-right">
                  <h3 className="text-lg font-semibold text-gray-500 uppercase tracking-wide">Confidence</h3>
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-500 uppercase tracking-wide">Key Patterns Identified</h3>
              <div className="mt-2 flex flex-wrap gap-2">
                {result.patterns.length > 0 ? result.patterns.map((pattern, index) => (
                    <span key={index} className="bg-gray-200 text-cyan-800 text-sm font-medium px-3 py-1 rounded-full">{pattern}</span>
                )) : <p className="text-gray-500">No specific patterns identified.</p>}
              </div>
            </div>
